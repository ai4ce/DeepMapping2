...
result = torch.zeros(pose_est.shape[0], pose_est.shape[0], dtype = torch.float).cpu()
...
if angle_contraint:
    save_name = os.path.join(checkpoint_dir,'bestn_w_ori_matrix.npy')
    np.save(save_name,result)
else:
    save_name = os.path.join(checkpoint_dir,'bestn_wo_ori_matrix.npy')
    np.save(save_name,result)
