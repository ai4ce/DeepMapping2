...
import utils
import loss
from models import DeepMapping_AVD, DeepMapping_AVD_ehcd, DeepMapping_AVD_clrg
...
dataset = AVD(opt.data_dir,opt.traj,opt.subsample_rate, trans_by_pose=init_pose)
...
elif opt.network == 'augmentation':
    model = DeepMapping_AVD_aug(loss_fn=loss_fn,n_samples=opt.n_samples).to(device)
elif opt.network == 'enhanced':
    model = DeepMapping_AVD_ehcd(loss_fn=loss_fn,n_samples=opt.n_samples).to(device)
elif opt.network == 'aug+enhanced':
    model = DeepMapping_AVD_aug_ehcd(loss_fn=loss_fn,n_samples=opt.n_samples).to(device)
else:
    assert()
