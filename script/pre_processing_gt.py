...
for idx in tqdm(range(n_pc-1)):
    ...
    pose_diff = gt_pose[idx+1] - gt_pose[idx]
    t_diff = pose_diff[:3]
    R00 = utils.ang2mat_np(gt_pose[idx, 3:])
    R0 = utils.ang2mat_np(gt_pose[idx+1, 3:])
    R_inv = np.linalg.inv(R00)
    R0 = np.matmul(R_inv, R0)
    t0 = np.matmul(R_inv, t_diff)
    if idx == 0: 
        R_cum = R0
        t_cum = t0
    else:
...
for idx in tqdm(range(n_pc)):
    for group_idx in range(1, opt.group_size):
        if opt.mode == "icp":
            ...
            pose_diff = gt_pose[group_matrix[idx, 0]] - gt_pose[group_matrix[idx, group_idx]]
            t_diff = pose_diff[:3]
            R00 = utils.ang2mat_np(gt_pose[group_matrix[idx, 0], 3:])
            R0 = utils.ang2mat_np(gt_pose[group_matrix[idx, group_idx], 3:])
            R_inv = np.linalg.inv(R00)
            R = np.matmul(R_inv, R0)
            t = np.matmul(R_inv, t_diff)
