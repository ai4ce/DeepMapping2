...
parser.add_argument('--mode', type=str, default=None, help='the method that gets best n')
...
loader = DataLoader(dataset,batch_size=opt.batch_size,shuffle=False, num_workers=2)
if opt.mode.isdigit():
    best_matrix = np.load("../data/best_n_trained_"+opt.mode+".npy")
    print("best_matrix: "+"../data/best_n_trained_"+opt.mode+".npy")
else:
    best_matrix = np.load("../data/gt_loop_closure.npy")
    print("best_matrix: "+"../data/gt_loop_closure.npy")
...
for epoch in range(opt.n_epochs):
    ...
    for index,(obs_batch,valid_pt,pose_batch) in enumerate(loader):
        obs_batch_all = obs_batch.to(device)
        valid_pt_all = valid_pt.to(device)
        pose_batch_all = pose_batch.to(device)

    for i in range(pose_batch_all.shape[0]):
        matrix_list = best_matrix[i]
        obs_batch_ = obs_batch_all[matrix_list]
        valid_pt_ = valid_pt_all[matrix_list]
        pose_batch_ = pose_batch_all[matrix_list]
        loss = model(obs_batch_,valid_pt_,pose_batch_)
